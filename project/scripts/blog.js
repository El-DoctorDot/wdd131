...
function renderSearchResults() {
    ...
    resultsGrid.innerHTML = filteredCars.slice(0, 6).map(car => createCarCard(car, false)).join('');
    ...
    if (filteredCars.length > 6) {
        const observer = new IntersectionObserver((entries) => {
            if (entries[0].isIntersecting) {
                
                resultsGrid.innerHTML += filteredCars.slice(6, 12).map(car => createCarCard(car, false)).join('');
                resultsGrid.querySelectorAll('.favorite-btn').forEach(btn => {
                    btn.addEventListener('click', (e) => {
                        const carId = e.target.dataset.carId;
                        toggleFavorite(carId);
